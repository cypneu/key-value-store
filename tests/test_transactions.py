...
def test_multi_returns_ok(conn):
    assert exec_command(conn, "MULTI") == "OK"


def test_exec_without_multi_errors(conn):
    with pytest.raises(RuntimeError) as excinfo:
        exec_command(conn, "EXEC")
    assert str(excinfo.value) == "ERR EXEC without MULTI"


def test_empty_transaction_executes_and_resets(conn):
    assert exec_command(conn, "MULTI") == "OK"
...
def test_commands_are_queued_until_exec(conn, make_key):
    key = make_key("queued")
    ...
    with socket.create_connection(("127.0.0.1", 6379), timeout=2.0) as other:
        assert exec_command(other, "GET", key) is None

    assert isinstance(exec_command(conn, "EXEC"), list)


def test_exec_runs_all_queued_commands_and_returns_results(conn, make_key):
    foo = make_key("foo")
    bar = make_key("bar")
...
def test_nested_multi_errors(conn):
    assert exec_command(conn, "MULTI") == "OK"
    with pytest.raises(RuntimeError) as excinfo:
...
def test_discard_aborts_transaction(conn, make_key):
...
def test_exec_collects_errors_and_continues(conn, make_key):
    foo = make_key("foo")
    bar = make_key("bar")
...
def test_multiple_transactions_have_separate_queues(conn, make_key):
