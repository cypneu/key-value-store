...
def test_echo(master):
    with master.client() as conn:
        assert exec_command(conn, "ECHO", "hello") == "hello"


def test_set_and_get(master, make_key):
    with master.client() as conn:
...
def test_incr_on_existing_integer(master, make_key):
    with master.client() as conn:
        key = make_key("incr")
...
def test_incr_on_missing_sets_to_one(master, make_key):
    with master.client() as conn:
...
def test_incr_on_non_integer_value_returns_error(master, make_key):
    with master.client() as conn:
        key = make_key("incr_non_integer")
        assert exec_command(conn, "SET", key, "xyz") == "OK"
        with pytest.raises(
            RuntimeError, match="ERR value is not an integer or out of range"
        ):
            exec_command(conn, "INCR", key)


def test_get_absent_returns_null(master, make_key):
    with master.client() as conn:
...
def test_set_with_px_expiry(master, make_key, px_token):
    with master.client() as conn:
        key = make_key("expiry")
...
def test_set_overwrites_existing_list(master, make_key):
    with master.client() as conn:
...
def test_get_returns_wrongtype_for_list(master, make_key):
    with master.client() as conn:
