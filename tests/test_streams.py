...
def test_xadd_returns_entry_id(master, make_key):
...
def test_type_reports_stream_for_stream_key(master, make_key):
    with master.client() as conn:
...
def test_xadd_returns_wrongtype_for_existing_string(master, make_key):
    with master.client() as conn:
...
def test_xadd_returns_wrongtype_for_existing_list(master, make_key):
    with master.client() as conn:
...
def test_xadd_rejects_non_incrementing_id(master, make_key):
    with master.client() as conn:
...
def test_xadd_rejects_zero_id(master, make_key):
    with master.client() as conn:
...
def test_xadd_auto_generates_sequence(master, make_key):
    with master.client() as conn:
...
def test_xadd_auto_generates_full_id(master, make_key):
    with master.client() as conn:
...
def test_xrange_returns_entries(master, make_key):
    with master.client() as conn:
...
def test_xrange_allows_dash_start(master, make_key):
    with master.client() as conn:
...
def test_xrange_allows_plus_end(master, make_key):
    with master.client() as conn:
...
def test_xread_returns_entries(master, make_key):
    with master.client() as conn:
...
def test_xread_returns_none_when_no_entries(master, make_key):
    with master.client() as conn:
...
def test_xread_multiple_streams(master, make_key):
    with master.client() as conn:
...
def test_xread_block_returns_entry(master, make_key):
    key = make_key("stream_xread_block")
    replies = queue.Queue()

    with master.client() as seed_conn:
        assert exec_command(seed_conn, "XADD", key, "0-1", "temperature", "96") == "0-1"

    def reader():
        with master.client() as sock:
            replies.put(
...
def test_xread_block_multiple_keys_returns_single_stream(master, make_key):
...
def test_xread_block_zero_waits_indefinitely(master, make_key):
...
def test_xread_block_timeout_returns_null(master, make_key):
...
def test_xread_block_latest_id_waits_for_new_entries(master, make_key):
